...
# settings
sample_submission_path = "/opt/ml/input/code/submission/sample_submission.csv"
...
def restore_file_name(file_name):
    if "test_1" in file_name:
        return "batch_01_vt/" + file_name[11:]
    if "test_2" in file_name:
        return "batch_02_vt/" + file_name[11:]
    if "test_3" in file_name:
        return "batch_03/" + file_name[11:]
...
def main():
    ...
    if rank == 0:
        if args.out:
            ...
            # 데이터 셋에서 순서대로 이미지 불러오기
            imgs = [image for img in dataset for image in img["img"]]
            # 데이터 셋에서 순서대로 파일명 불러와서 변경하기: test/test10123.jpg -> batch_01_vt/0123.jpg
            file_names = [
                restore_file_name(e.data["filename"][35:])
                for img in dataset
                for e in img["img_metas"]
            ]
            # 모델 prediction 결과 전처리
            oms = np.stack(results)
            # (256, 256)으로 reshape하고, mask 추출
            temp_mask = []
            for img, mask in tqdm.tqdm(zip(np.stack(imgs), oms)):
                transformed = resize(image=img, mask=mask)
                mask = transformed["mask"]
                temp_mask.append(mask)
            preds_array = np.empty((0, size * size), dtype=np.long)
            # (sample수, 256 x 256)으로 reshape
            oms = np.array(temp_mask)
            oms = oms.reshape([oms.shape[0], size * size]).astype(int)
            preds = np.vstack((preds_array, oms))
            # sample_submisson.csv 열기
            submission = pd.read_csv(sample_submission_path, index_col=None)
            # PredictionString 대입
